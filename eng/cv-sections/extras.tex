\cvsection{Talks, Workshops \& Hackathons}

\begin{cventries}

\cventry
{Speaker} % Role
{Nodo Serano Hackathon} % Event
{Tandil, Argentina} % Location
{Oct. 2025}
{
\begin{cvitems}
\item {Introduced developers to Ethereum and smart contract development with Solidity}
\end{cvitems}
}

\cventry
{Mentor} % Role
{Infinite Hackathon} % Event
{Bogota, Colombia} % Location
{Oct. 2022}
{
\begin{cvitems}
\item {Mentored teams building intent related dApps using Ethereum and decentralized protocols}
\item {Provided guidance on Brink Smart Contracts integration and usage, API interactions}
\item {Gatered usage feedback to improve Brink developer experience}
\end{cvitems}
}

\cventry
{Speaker} % Role
{Identihack} % Event
{Remote, Latin America} % Location
{Jul. 2021 – Aug. 2021}
{
\begin{cvitems}
\item {Delivered a technical talk introducing Self-Sovereign Identity (SSI)}
\item {Presented Proyecto DIDI's architecture and challenges in decentralized identity}
\end{cvitems}
}

\cventry
{Teacher} % Role
{Beereal} % Company
{Tandil, Argentina} % Location
{Mar. 2020 – Apr. 2020}
{
\begin{cvitems}
\item {Taught workshops on Ethereum smart contract development with Solidity}
\item {Introduced core blockchain concepts to developers with no prior experience}
\end{cvitems}
}

\cventry
{Organizer \& Speaker} % Role
{Tandil Dev Meetup} % Event
{Tandil, Argentina} % Location
{2019}
{
\begin{cvitems}
\item {Organized and spoke at local tech meetups focused on blockchain and web development}
\item {Delivered sessions on Solidity, smart contract development, and basic React/Node.js}
\item {Led a hands-on Solidity workshop for developers}
\end{cvitems}
}

\cventry
{Mentor} % Role
{WindingTree Hackathon} % Event
{Prague, Czech Republic} % Location
{Oct. 2018}
{
\begin{cvitems}
\item {Mentored teams building travel-tech dApps using Ethereum and decentralized protocols}
\item {Provided guidance on API integration, smart contracts and design decisions}
\end{cvitems}
}

\cventry
{Hacker} % Role
{CryptoLife Hackathon} % Event
{Prague, Czech Republic} % Location
{Oct. 2018}
{
\begin{cvitems}
\item {Participated in Web3 hackathon focused on Ethereum infrastructure}
\item {Attended workshops on blockchain development and UX}
\end{cvitems}
}

\cventry
{Hacker} % Role
{ETH Buenos Aires} % Event
{Buenos Aires, Argentina} % Location
{May 2018}
{
\begin{cvitems}
\item {Developed an oracles composition dApp using Solidity}
\end{cvitems}
}

\end{cventries}
